...
import type { Category } from "../slice/categoriesSlice";
...
export async function fetchCategory(id: number | string): Promise<Category> {
  const res = await apiGet<Category>(`/api/categories/${id}`);
  if (res.error) throw new Error(res.error);
  return res.data!;
}
